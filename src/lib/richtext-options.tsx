...
import { BlogParagraph } from "@/components/elements";
  ...
  renderNode: {
    ...
    [BLOCKS.EMBEDDED_ENTRY]: (node: any, children) => {
      ...
      if (sys.contentType.sys.id === "codeblocks") {
        return <pre>{fields.codeblock}</pre>;
      }
    ...
    [BLOCKS.HEADING_1]: (node: any) => {
      ...
      return (
        <h1 className="flex items-center group mt-14 mr-0 mb-7 ml-0 text-3xl">
          ...
          <a
            className="mr-2 not-prose text-foreground no-underline font-bold text-3xl"
            href={`#${headingId}`}
          >
    ...
    [BLOCKS.HEADING_2]: (node: any) => {
      ...
      return (
        <h2 className="flex items-center group mt-14 mr-0 mb-5 ml-0 text-xl">
          <span id={headingId}></span>
          <a
            className="mr-2 not-prose text-foreground no-underline font-bold text-xl"
            href={`#${headingId}`}
          >
            {node.content[0].value}
          </a>
    ...
    [BLOCKS.HEADING_3]: (node: any) => {
      ...
      return (
        <h3 className="flex items-center group mt-14 mr-0 mb-4 ml-0 text-lg">
          <span id={headingId}></span>
          <a
            className="mr-2 not-prose text-foreground no-underline font-bold text-lg"
            href={`#${headingId}`}
          >
            {node.content[0].value}
          </a>
    ...
    [BLOCKS.HEADING_4]: (node: any) => {
      ...
      return (
        <h4 className="flex items-center group mt-14 mr-0 mb-8 ml-0">
          <span id={headingId}></span>
          <a
            className="mr-2 not-prose text-foreground no-underline font-bold"
            href={`#${headingId}`}
          >
            {node.content[0].value}
          </a>
    ...
    [INLINES.HYPERLINK]: ({ data }, children) => {
      return (
        <a
          ...
          href={data?.uri}
          target={`${
            data?.uri.startsWith(siteConfig.url) ? "_self" : "_blank"
          }`}
          rel={`${
            data?.uri.startsWith(siteConfig.url) ? "" : "noopener noreferrer"
          }`}
        >
          {children}
        </a>
