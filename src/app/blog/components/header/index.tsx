  ...
  return (
      <PageHeader className="h-fit md:h-fit items-start w-full md:w-[736px] lg:w-[948px] md:px-0">
        ...
        <div className="flex flex-col items-start gap-4 w-full">
          ...
          <button onClick={() => router.push('/blog')} className='bg-primary hover:bg-primary/80 p-4 rounded'>All</button>
            {tags.map(tag => <button key={tag.id} onClick={() => handleSelectTag(tag.id)} className='bg-primary hover:bg-primary/80 p-4 rounded'>{tag.title}</button>)}
          </div>
        </div>
      </PageHeader>
  )
